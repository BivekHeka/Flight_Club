...
class FlightSearch:
    ...
    def get_destination_code(self, city_name):
        # Return first 3 letters of city as a dummy IATA code
        return city_name[:3].upper()

    def check_flights(self, origin_city_code, destination_city_code, from_time, to_time, is_direct=True):
        # Return a mocked flight dictionary like Amadeus response
        stops = 0 if is_direct else 1
        price = 100 + stops * 50  # direct cheaper, stopover adds 50
        return {
            "data": [
                {
                    "price": {"grandTotal": price},
                    "itineraries": [
                        {"segments": [{"departure": {"iataCode": origin_city_code, "at": from_time.strftime("%Y-%m-%dT10:00:00")},
                                       "arrival": {"iataCode": destination_city_code, "at": from_time.strftime("%Y-%m-%dT14:00:00")}}]},
                        {"segments": [{"departure": {"iataCode": destination_city_code, "at": to_time.strftime("%Y-%m-%dT10:00:00")},
                                       "arrival": {"iataCode": origin_city_code, "at": to_time.strftime("%Y-%m-%dT14:00:00")}}]}
                    ]
                }
            ]
        }